...
```bash
bun run start          # Start Metro bundler on port 8087
...
Custom `NetworkClient` implementation (fetch-based) — does not use the web DI layer. Configured via `ApiContext` which provides the genuivo_client and QueryClient.
...
| ---------------------------------- | -------------------- | ---------------- |
| `EXPO_PUBLIC_API_URL`              | Backend API URL      | `localhost:8026` |
...
- **genuivo_types** — Shared type definitions
- **genuivo_client** — API client SDK
- **genuivo_lib** — Business logic library with `useHistoriesManager`
- **genuivo_api** — Backend server this app communicates with
- **genuivo_app** — Web counterpart of this mobile app; shares genuivo_client, genuivo_lib, and genuivo_types
...
- The prestart script merges `.env` files -- environment changes require restarting Metro (port 8087)
- Metro runs on port `8087` (not the default 8081) -- ensure no port conflicts
