...
import type { History } from '@sudobility/genuivo_types';
import { trackScreenView, trackButtonClick, trackError, trackEvent } from '@/analytics';
...
export default function HistoriesScreen({
  ...
  const handleAddHistory = useCallback(async () => {
    ...
    trackButtonClick('submit_history');
    try {
      await createHistory({
        datetime: new Date().toISOString(),
        value,
      });
      trackEvent('history_created');
      setNewValue('');
